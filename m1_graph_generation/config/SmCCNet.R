...
omics_files <- strsplit(args[3], ",")[[1]]
data_type <- strsplit(args[4], ",")[[1]]
kfold <- as.numeric(args[5])
summarization <- args[6]
seed <- as.numeric(args[7])
saving_dir <- args[8]

# To make sure data is being passed correctly
print(paste("Phenotype file:", phenotype_file))
print(paste("Omics files:", paste(omics_files, collapse = ", ")))
print(paste("Data types:", paste(data_type, collapse = ", ")))
...
print(paste("Phenotype factor levels:", levels(phenotype_vector)))
...
for (i in seq_along(omics_files)) {
  ...
  if (any(is.na(omics_data_numeric)) || any(is.infinite(omics_data_numeric))) {
    print("Warning: NAs or infinite values detected in omics data after alignment.")
    # Handle NAs as needed (e.g., impute, remove samples)
  }
...
# Proceed with SmCCNet execution
...
# Setting variables for the directory and subdirectory
subdirectory <- "CSV_Files"

# Creating the subdirectory
dir.create(file.path(saving_dir, subdirectory), showWarnings = FALSE)

# Load and write the global network to CSV
load(file.path(saving_dir, "globalNetwork.Rdata"))
write.csv(globalNetwork$AdjacencyMatrix, file = file.path(saving_dir, subdirectory, "global_network.csv"))

# Process subnetworks
subnetworks <- list.files(path = saving_dir, pattern = "^size_.*\\.Rdata$", full.names = TRUE)
...
for (subnet in subnetworks) {
  ...
  # Write to CSV
  write.csv(adjacency_matrix, file = file.path(saving_dir, subdirectory, paste0(file_name_clean, ".csv")))
}
