...
if (length(args) != 7) {
  stop("Exactly 7 arguments must be supplied: phenotype_file, omics_file, data_types, kfold, summarization, seed, output_dir")
}
...
write.csv(globalNetwork$AdjacencyMatrix, file = file.path(saving_dir, "global_network.csv"))
...
for (subnet in subnetworks) {
  ...
  # Write to CSV
  write.csv(adjacency_matrix, file = file.path(saving_dir, paste0(file_name_clean, ".csv")))
}
